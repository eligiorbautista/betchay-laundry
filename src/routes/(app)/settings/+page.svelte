...
<div class="p-4 lg:p-6 w-full bg-gray-50 min-h-screen">
	...
	<div class="mb-6 md:mb-8">
		<div class="flex items-center gap-3 mb-2">
			<Settings class="w-6 md:w-8 h-6 md:h-8 text-brand-800" />
			<h1 class="text-2xl md:text-3xl font-bold text-brand-900">Settings</h1>
		</div>
	...
	<div class="mb-8">
		<div class="border-b border-gray-200">
			...
			<nav class="hidden md:flex -mb-px space-x-8">
				<button
					class="py-3 px-1 border-b-2 font-medium text-sm transition-colors {activeTab === 'profile' ? 'border-brand-800 text-brand-800' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
				...
				<button
					class="py-3 px-1 border-b-2 font-medium text-sm transition-colors {activeTab === 'security' ? 'border-brand-800 text-brand-800' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
			...
			<nav class="md:hidden -mb-px grid grid-cols-2 gap-2 p-2">
				<button
					class="py-3 px-3 font-medium text-xs transition-colors border-b-2 {activeTab === 'profile' ? 'text-brand-800 bg-gray-50 border-brand-800' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50 border-transparent'}"
					on:click={() => activeTab = 'profile'}
				>
				...
				<button
					class="py-3 px-3 font-medium text-xs transition-colors border-b-2 {activeTab === 'security' ? 'text-brand-800 bg-gray-50 border-brand-800' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50 border-transparent'}"
					on:click={() => activeTab = 'security'}
				>
					<div class="flex flex-col items-center gap-1">
						<Shield class="w-4 h-4" />
	...
	<div class="max-w-4xl w-full">
		{#if activeTab === 'profile'}
			...
			<div class="bg-white rounded-lg shadow-sm border border-gray-200">
				<div class="px-4 md:px-6 py-4 border-b border-gray-200">
					<h2 class="text-lg font-semibold text-brand-900">Profile Information</h2>
				...
				<div class="p-4 md:p-6 space-y-6 md:space-y-8">
					<!-- Account Information -->
					<div>
						<h3 class="text-lg font-medium text-brand-900 mb-4">Account Information</h3>
						<div class="bg-gray-50 rounded-lg p-4">
							<div>
								<label for="email-display" class="block text-sm font-medium text-gray-700 mb-1">Email Address</label>
								<p id="email-display" class="text-sm text-brand-900 font-medium">{userProfile.email}</p>
							</div>
						</div>
		...
		{:else if activeTab === 'security'}
			...
			<div class="bg-white rounded-lg shadow-sm border border-gray-200">
				<div class="px-4 md:px-6 py-4 border-b border-gray-200">
					<h2 class="text-lg font-semibold text-brand-900">Security Settings</h2>
					<p class="text-sm text-gray-600">Manage your password and security preferences.</p>
				</div>
				<div class="p-4 md:p-6 space-y-6 md:space-y-8">
					<!-- Change Password Section -->
					<div>
						<h3 class="text-lg font-medium text-brand-900 mb-4">Change Password</h3>
						<div class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
							<div class="flex flex-col sm:flex-row items-start gap-3">
								<Lock class="w-5 h-5 text-brand-800 mt-0.5 flex-shrink-0" />
								<div>
									<h4 class="text-sm font-medium text-brand-900">Password Security</h4>
						...
						<form on:submit|preventDefault={handlePasswordChange} class="space-y-6">
							...
							<div class="flex flex-col sm:flex-row justify-end pt-6">
								<button
									...
									class="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-brand-500 text-white rounded-lg hover:bg-brand-900 focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium"
								>
									{#if loading}
					...
					<div class="border-t border-gray-200 pt-6 md:pt-8">
						<h3 class="text-lg font-medium text-brand-900 mb-4">Two-Factor Authentication</h3>
						<div class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
							<div class="flex flex-col sm:flex-row items-start gap-3">
								<Shield class="w-5 h-5 text-brand-800 mt-0.5 flex-shrink-0" />
								<div>
									<h4 class="text-sm font-medium text-brand-900">Enhanced Security</h4>
									<p class="text-sm text-gray-700 mt-1">
						...
						<div class="space-y-4">
							<div class="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-gray-200 rounded-lg gap-4">
								<div>
									<h4 class="text-sm font-medium text-brand-900">Email Verification</h4>
